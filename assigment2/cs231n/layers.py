...
def affine_backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], -1).T.dot(dout) #dout = NxM, x= N,d1,d2,..dk, dw = D,M
    db = dout.sum(axis=0)  # for some reason (or not) it is not divided by the number of examples!
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward(dout, cache):
    ...
    dvar = dvar_eps
    dsxmu = dvar * np.ones_like(dx, dtype=dtype) / N
    dxmu1 = dxnorm * inv
    dxmu2 = dsxmu * 2 * xmu
    dxmu = dxmu1 + dxmu2
    dmu = - dxmu.sum(axis=0)
    dx1 = dxmu
    dx2 = dmu * np.ones_like(dx, dtype=dtype) / N
    dx = dx1 + dx2
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        out = x * mask / (1 - p) # to account for decreased signals strengths in neurons (inverted implementation)
        pass
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    out = np.zeros((x.shape[0], x.shape[1], dim_p_h, dim_p_w))
    for h in range(dim_p_h):
        for w in range(dim_p_w):
            x_seg = x[:, :, h * stride:h * stride + p_h, w * stride:w * stride + p_w]
            out[:, :, h, w] = x_seg.max(axis=(2, 3))
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_flat = dout_trans.reshape((-1, C))
